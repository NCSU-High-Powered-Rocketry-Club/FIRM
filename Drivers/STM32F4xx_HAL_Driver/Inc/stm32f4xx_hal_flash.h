 ...
 */
typedef struct {
    __IO FLASH_ProcedureTypeDef ProcedureOnGoing; /*Internal variable to indicate which procedure is
                                                     ongoing or not in IT context*/

    __IO uint32_t
        NbSectorsToErase; /*Internal variable to save the remaining sectors to erase in IT context*/

    __IO uint8_t VoltageForErase; /*Internal variable to provide voltage range selected by user in
                                     IT context*/
...
#define __HAL_FLASH_SET_LATENCY(__LATENCY__)                                                       \
    (*(__IO uint8_t*)ACR_BYTE0_ADDRESS = (uint8_t)(__LATENCY__))
...
#define IS_FLASH_TYPEPROGRAM(VALUE)                                                                \
    (((VALUE) == FLASH_TYPEPROGRAM_BYTE) || ((VALUE) == FLASH_TYPEPROGRAM_HALFWORD) ||             \
     ((VALUE) == FLASH_TYPEPROGRAM_WORD) || ((VALUE) == FLASH_TYPEPROGRAM_DOUBLEWORD))
/**
 * @}
 */
