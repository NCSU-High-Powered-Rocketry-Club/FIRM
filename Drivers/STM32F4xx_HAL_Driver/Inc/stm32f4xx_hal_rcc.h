...
/**
 * @brief  RCC Internal/External Oscillator (HSE, HSI, LSE and LSI) configuration structure
 * definition
 */
typedef struct {
    ...
    uint32_t HSICalibrationValue; /*!< The HSI calibration trimming value (default is
...
/** @defgroup RCC_AHB1_Peripheral_Clock_Enable_Disable_Status AHB1 Peripheral Clock Enable Disable
 * Status
...
/** @defgroup RCC_APB1_Peripheral_Clock_Enable_Disable_Status APB1 Peripheral Clock Enable Disable
 * Status
...
/** @defgroup RCC_APB2_Peripheral_Clock_Enable_Disable_Status APB2 Peripheral Clock Enable Disable
 * Status
 ...
 *            @arg @ref RCC_RTCCLKSOURCE_HSE_DIVX HSE divided by X selected as RTC clock (X can be
 * retrieved thanks to @ref __HAL_RCC_GET_RTC_HSE_PRESCALER()
...
#define __HAL_RCC_RTC_CLKPRESCALER(__RTCCLKSource__)                                               \
    (((__RTCCLKSource__) & RCC_BDCR_RTCSEL) == RCC_BDCR_RTCSEL)                                    \
        ? MODIFY_REG(RCC->CFGR, RCC_CFGR_RTCPRE, ((__RTCCLKSource__) & 0xFFFFCFFU))                \
 ...
 *            @arg RCC_MCO2SOURCE_PLLI2SCLK: PLLI2S clock selected as MCO2 source, available for all
 * STM32F4 devices except STM32F410xx
 *            @arg RCC_MCO2SOURCE_I2SCLK: I2SCLK clock selected as MCO2 source, available only for
 * STM32F410Rx devices
...
#define __HAL_RCC_MCO2_CONFIG(__MCOCLKSOURCE__, __MCODIV__)                                        \
    MODIFY_REG(RCC->CFGR, (RCC_CFGR_MCO2 | RCC_CFGR_MCO2PRE),                                      \
               ((__MCOCLKSOURCE__) | ((__MCODIV__) << 3U)));
/**
 * @}
 */
...
#define IS_RCC_SYSCLKSOURCE(SOURCE)                                                                \
    (((SOURCE) == RCC_SYSCLKSOURCE_HSI) || ((SOURCE) == RCC_SYSCLKSOURCE_HSE) ||                   \
     ((SOURCE) == RCC_SYSCLKSOURCE_PLLCLK) || ((SOURCE) == RCC_SYSCLKSOURCE_PLLRCLK))
...
#define IS_RCC_HCLK(HCLK)                                                                          \
    (((HCLK) == RCC_SYSCLK_DIV1) || ((HCLK) == RCC_SYSCLK_DIV2) || ((HCLK) == RCC_SYSCLK_DIV4) ||  \
     ((HCLK) == RCC_SYSCLK_DIV8) || ((HCLK) == RCC_SYSCLK_DIV16) ||                                \
     ((HCLK) == RCC_SYSCLK_DIV64) || ((HCLK) == RCC_SYSCLK_DIV128) ||                              \
     ((HCLK) == RCC_SYSCLK_DIV256) || ((HCLK) == RCC_SYSCLK_DIV512))
...
#define IS_RCC_MCODIV(DIV)                                                                         \
    (((DIV) == RCC_MCODIV_1) || ((DIV) == RCC_MCODIV_2) || ((DIV) == RCC_MCODIV_3) ||              \
     ((DIV) == RCC_MCODIV_4) || ((DIV) == RCC_MCODIV_5))
