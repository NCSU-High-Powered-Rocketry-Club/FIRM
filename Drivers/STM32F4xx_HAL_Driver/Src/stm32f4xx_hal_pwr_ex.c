...
selected. Refer to the datasheets for more details.
...
registers and internal SRAM. 2 operating modes are available:
         (++) Normal mode: the 1.2V domain is preserved in nominal leakage mode. This mode is only
              ...
              low voltage mode.
         (++) Under-drive mode: the 1.2V domain is preserved in reduced leakage mode. This mode is
only available when the main regulator or the low power regulator is in low voltage mode.
 ...
 * @retval None
 */
void HAL_PWREx_EnableFlashPowerDown(void) { *(__IO uint32_t*)CR_FPDS_BB = (uint32_t)ENABLE; }

/**
 * @brief Disables the Flash Power Down in Stop mode.
 * @retval None
 */
void HAL_PWREx_DisableFlashPowerDown(void) { *(__IO uint32_t*)CR_FPDS_BB = (uint32_t)DISABLE; }
 ...
 */
uint32_t HAL_PWREx_GetVoltageRange(void) { return (PWR->CR & PWR_CR_VOS); }
 ...
 *          This parameter can be one of the following values:
 *            @arg PWR_REGULATOR_VOLTAGE_SCALE1: Regulator voltage output range 1 mode,
 *                                               the maximum value of fHCLK is 168 MHz. It can be
 * extended to 180 MHz by activating the over-drive mode.
 *            @arg PWR_REGULATOR_VOLTAGE_SCALE2: Regulator voltage output range 2 mode,
 *                                               the maximum value of fHCLK is 144 MHz. It can be
 * extended to, 168 MHz by activating the over-drive mode.
 ...
 * @note This API forces the PLL state ON to allow the possibility to configure the voltage scale 1
 * or 2.
...
#endif /* STM32F401xC || STM32F401xE || STM32F410xx || STM32F411xE || STM32F412Zx || STM32F412Rx   \
 ...
 * @note   This function can be used only for
 * STM32F42xx/STM32F43xx/STM32F446xx/STM32F469xx/STM32F479xx devices. This mode allows the CPU and
 * the core logic to operate at a higher frequency than the normal mode for a given voltage scaling
 ...
 *
 * @note   This mode is only available for
 * STM32F42xxx/STM32F43xxx/STM32F446xx/STM32F469xx/STM32F479xx devices.
 *
