/**
  ...
  *** SD Card Initialization and configuration ***
  ...
  [..]
    ...
    SDIO Peripheral(STM32 side) and the SD Card, and put it into StandBy State (Ready for data
...
HAL_StatusTypeDef HAL_SD_WriteBlocks(SD_HandleTypeDef* hsd, uint8_t* pData, uint32_t BlockAdd,
    ...
    if (hsd->State == HAL_SD_STATE_READY) {
        ...
        dataremaining = config.DataLength;
#if defined(SDIO_STA_STBITERR)
        while (!__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_TXUNDERR | SDIO_FLAG_DCRCFAIL |
...
#else  /* SDIO_STA_STBITERR not defined */
        __HAL_SD_ENABLE_IT(
            hsd, (SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_RXOVERR | SDIO_IT_DATAEND));
#endif /* SDIO_STA_STBITERR */
...
#if defined(SDIO_STA_STBITERR)
        __HAL_SD_ENABLE_IT(
            hsd, (SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_TXUNDERR | SDIO_IT_STBITERR));
#else  /* SDIO_STA_STBITERR not defined */
        __HAL_SD_ENABLE_IT(hsd, (SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_TXUNDERR));
#endif /* SDIO_STA_STBITERR */
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SD_Erase(SD_HandleTypeDef* hsd, uint32_t BlockStartAdd,
    ...
    if (hsd->State == HAL_SD_STATE_READY) {
        ...
        if ((SDIO_GetResponse(hsd->Instance, SDIO_RESP1) & SDMMC_CARD_LOCKED) ==
            SDMMC_CARD_LOCKED) {
            /* Clear all the static flags */
            __HAL_SD_CLEAR_FLAG(hsd, SDIO_STATIC_FLAGS);
...
void HAL_SD_IRQHandler(SD_HandleTypeDef* hsd) {
    ...
    if ((__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_RXFIFOHF) != RESET) &&
    ...
    else if ((__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_TXFIFOHE) != RESET) &&
             ((context & SD_CONTEXT_IT) != 0U)) {
        SD_Write_IT(hsd);
    }

#if defined(SDIO_STA_STBITERR)
    else if (__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_RXOVERR |
...
HAL_StatusTypeDef HAL_SD_GetCardCSD(SD_HandleTypeDef* hsd, HAL_SD_CardCSDTypeDef* pCSD) {
    ...
    if (hsd->SdCard.CardType == CARD_SDSC) {
        pCSD->DeviceSize =
            (((hsd->CSD[1] & 0x000003FFU) << 2U) | ((hsd->CSD[2] & 0xC0000000U) >> 30U));
        ...
        /* Byte 7 */
        pCSD->DeviceSize =
            (((hsd->CSD[1] & 0x0000003FU) << 16U) | ((hsd->CSD[2] & 0xFFFF0000U) >> 16U));
...
HAL_StatusTypeDef HAL_SD_GetCardStatus(SD_HandleTypeDef* hsd, HAL_SD_CardStatusTypeDef* pStatus) {
    ...
    if (errorstate != HAL_SD_ERROR_NONE) {
        ...
        pStatus->CardType = (uint16_t)(((sd_status[0] & 0x00FF0000U) >> 8U) |
...
static uint32_t SD_PowerON(SD_HandleTypeDef* hsd) {
    ...
    while ((count < SDMMC_MAX_VOLT_TRIAL) && (validvoltage == 0U)) {
        ...
        errorstate = SDMMC_CmdAppOperCommand(
            hsd->Instance, SDMMC_VOLTAGE_WINDOW_SD | SDMMC_HIGH_CAPACITY | SD_SWITCH_1_8V_CAPACITY);
        if (errorstate != HAL_SD_ERROR_NONE) {
            return HAL_SD_ERROR_UNSUPPORTED_FEATURE;
        }
    ...
    if ((response & SDMMC_HIGH_CAPACITY) ==
        SDMMC_HIGH_CAPACITY) /* (response &= SD_HIGH_CAPACITY) */
    {
