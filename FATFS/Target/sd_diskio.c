    ...
    if (!((uint32_t)buff & 0x3)) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (ret == MSD_OK) {
                ...
                /*
                 *
                 * invalidate the scratch buffer before the next read to get the actual data instead
                 * of the cached one
                 */
...
#if (ENABLE_SD_DMA_CACHE_MAINTENANCE == 1)
        /*
         * invalidate the scratch buffer before the next write to get the actual data instead of the
         * cached one
         */
        SCB_InvalidateDCache_by_Addr((uint32_t*)scratch, BLOCKSIZE);
#endif
 ...
 * @retval None
 */
void BSP_SD_WriteCpltCallback(void) { WriteStatus = 1; }
 ...
 * @retval None
 */
void BSP_SD_ReadCpltCallback(void) { ReadStatus = 1; }

/* USER CODE BEGIN ErrorAbortCallbacks */
/*
==============================================================================================
  depending on the SD_HAL_Driver version, either the HAL_SD_ErrorCallback() or
HAL_SD_AbortCallback() or both could be defined, activate the callbacks below when suitable and
needed
==============================================================================================
