    ...
    with open(path, "rb") as f:
        ...
        try:
            while num_repeat_whitespace <= (max([BMP581_SIZE, ICM45686_SIZE, MMC5983MA_SIZE]) + 4):
                ...
                # read timestamp
                num_repeat_whitespace = 0
                clock_count = struct.unpack(
                    ">I", b"\x00" + characters[currentByte + 1 : currentByte + 4]
                )[0]
                ...
                if characters[currentByte - 4] == BMP581_ID:
                    ...
                    data = struct.unpack(
                        "<ffffff", characters[currentByte : currentByte + ICM45686_SIZE]
                    )

                    icm45686_data.append(
                        [timestamp_seconds, data[0], data[1], data[2], data[3], data[4], data[5]]
                    )
                    currentByte += ICM45686_SIZE

                elif characters[currentByte - 4] == MMC5983MA_ID:
                    data = struct.unpack(
                        "<fff", characters[currentByte : currentByte + MMC5983MA_SIZE]
                    )
        ...
        icm45686_df = pd.DataFrame(
            icm45686_data,
            columns=["timestamp", "accel_x", "accel_y", "accel_z", "gyro_x", "gyro_y", "gyro_z"],
        )
        mmc5983ma_df = pd.DataFrame(
            mmc5983ma_data, columns=["timestamp", "mag_x", "mag_y", "mag_z"]
        )
