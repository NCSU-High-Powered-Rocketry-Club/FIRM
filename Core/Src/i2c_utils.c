...
int i2c_read(I2C_HandleTypeDef* hi2c, uint8_t dev_addr, uint8_t reg_addr, uint8_t* buffer,
    ...
    return HAL_I2C_Mem_Read(hi2c, (uint16_t)(dev_addr << 1), (uint16_t)reg_addr, I2C_MEMADD_SIZE_8BIT, buffer, len, 100);
}

int i2c_write(I2C_HandleTypeDef* hi2c, uint8_t dev_addr, uint8_t reg_addr, uint8_t data) {
    return HAL_I2C_Mem_Write(hi2c, (uint16_t)(dev_addr << 1), (uint16_t)reg_addr, I2C_MEMADD_SIZE_8BIT, &data, 1, 100);
}
