...
void icm45686_convert_packet_normal_values(void) {
    ...
    // Z gyro raw = 524288 -> accel_z = 524288 / (131.072F * (180.0F / pi)) = 4000
    
    packet.gyroX_H = 0x00; packet.gyroX_L = 0x00;
    packet.gyroY_H = 0xF2; packet.gyroY_L = 0x2D;
    packet.gyroZ_H = 0x80; packet.gyroZ_L = 0x00;

    // 4 bits for accel, 4 bits for gyro (0baaaagggg)
    packet.x_vals_lsb = (0xF << 4) | (0x0 & 0x0F);
    packet.y_vals_lsb = (0x0 << 4) | (0x3 & 0x0F);
    packet.z_vals_lsb = (0x0 << 4) | (0x0 & 0x0F);
    ...
    float expected_gyro_z = 524288.0F / gyro_scale_factor;
    ...
    TEST_ASSERT_FLOAT_WITHIN(1e-6, expected_gyro_z, ret_packet.angular_rate_z);
}
