...
static DeviceProtocol select_protocol_from_settings(void) {
  ...
  if (settings->uart_transfer_enabled) {
    return PROTOCOL_UART;
  }
  if (settings->i2c_transfer_enabled) {
    return PROTOCOL_I2C;
  }
  if (settings->spi_transfer_enabled) {
    return PROTOCOL_SPI;
  }
  return PROTOCOL_USB;
}
...
uint32_t execute_command(CommandIdentifier identifier, uint8_t *data, uint32_t data_len,
  ...
  case CMDID_GET_CALIBRATION: {
    // Payload format: [Accel Calibration][Gyro Calibration][Mag Calibration][High-g Calibration] (in row-major)
