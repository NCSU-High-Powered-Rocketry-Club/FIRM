...
void ukf_state_transition_function(const float *sigmas, const float dt, const State state,
  ...
  float quat[4] = {sigmas[UKF_STATE_DIMENSION - 4], sigmas[UKF_STATE_DIMENSION - 3],
                   sigmas[UKF_STATE_DIMENSION - 2], sigmas[UKF_STATE_DIMENSION - 1]};
  ...
  if (state == MOTOR_BURN || state == COAST || state == DESCENT) {
    ...
    float delta_theta[3] = {sigmas[9] * dt, sigmas[10] * dt, sigmas[11] * dt};
    ...
    float accel_grav[3] = {sigmas[6] * GRAVITY_METERS_PER_SECOND_SQUARED,
