...
bool validate_message_crc16(uint16_t header, uint16_t identifier, uint32_t payload_length,
  ...
  uint16_t received_crc =
      payload_and_crc[payload_length] | ((uint16_t)payload_and_crc[payload_length + 1] << 8);

  // Calculate total message length for CRC verification
  uint32_t total_crc_length =
      sizeof(header) + sizeof(identifier) + sizeof(payload_length) + payload_length;
  ...
  memcpy(&temp_buffer[sizeof(header) + sizeof(identifier)], &payload_length,
         sizeof(payload_length));
  memcpy(&temp_buffer[sizeof(header) + sizeof(identifier) + sizeof(payload_length)],
...
bool usb_parse_message_meta(const uint8_t *meta_bytes, size_t meta_len, UsbMessageMeta *out) {
  ...
  memcpy(&out->payload_length, &meta_bytes[sizeof(out->header) + sizeof(out->identifier)],
         sizeof(out->payload_length));
  return true;
}
