...
static void send_response(Identifiers_t id, const void *response_data, uint16_t response_len) {
  TransmitFrame_t transmit_frame;

  if (response_len > (MAX_TRANSMIT_PAYLOAD_LEN - 1U) || queue_send == NULL) {
    return;
  }

  transmit_frame.payload_len = (uint16_t)(1U + response_len);
...
void dispatch_command(const uint8_t *command_bytes) {
  ...
  case ID_GET_DEVICE_INFO: {
    ...
    send_response(ID_GET_DEVICE_INFO, &device_info, sizeof(DeviceInfo_t));
    break;
  }
  ...
  case ID_GET_DEVICE_CONFIG: {
    ...
    send_response(ID_GET_DEVICE_CONFIG, &device_config, sizeof(DeviceConfig_t));
    break;
  }
  ...
  case ID_SET_DEVICE_CONFIG: {
    ...
    const bool success = settings_write_firm_settings(&updated_settings) == 0;
    send_response(ID_SET_DEVICE_CONFIG, &success, sizeof(success));
    break;
  }
  ...
  case ID_SET_MAG_CALIBRATON: {
    ...
    const bool success = settings_write_calibration(NULL, NULL, &new_mag_calibration, NULL) == 0;
    send_response(ID_SET_MAG_CALIBRATON, &success, sizeof(success));
    break;
  }
  ...
  case ID_SET_IMU_CALIBRATON: {
    ...
    const bool success =
        settings_write_calibration(&new_accel_calibration, &new_gyro_calibration, NULL, NULL) == 0;
    send_response(ID_SET_IMU_CALIBRATON, &success, sizeof(success));
    break;
  }

  case ID_GET_CALIBRATION:
    send_response(ID_GET_CALIBRATION, &settings->accel_cal, sizeof(Calibration_t) * 4);
    break;
